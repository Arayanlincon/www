package cn.wzq.shopping.utils.Redsi;
...
public class RedisShardedPool {
    ...
    //= PropertiesUtil.getProperty("redis2.host");
    ...
    //= Integer.parseInt(PropertiesUtil.getProperty("redis.maxTotal", "20"));

    //最大空闲连接数
    private static Integer maxIdle;
    //= Integer.parseInt(PropertiesUtil.getProperty("redis.maxIdle", "10"));

    //最小空闲连接数
    private static Integer minIdle;
    //= Integer.parseInt(PropertiesUtil.getProperty("redis.minIdle", "2"));

    //从连接池拿出时是否进行验证，true-验证，取出的redis连接一定可用
    private static Boolean testOnborrow;
    //= Boolean.parseBoolean(PropertiesUtil.getProperty("redis.testOnBorrow", "true"));

    //放回连接池时是否进行验证，true-验证，放回的redis连接一定可用
    private static Boolean testOnReturn;
    //= Boolean.parseBoolean(PropertiesUtil.getProperty("redis.testOnReturn", "false"));
